...
class EggApplication extends EggCore {
  ...
  dumpConfig() {
    ...
    const ignoreList = this.config.dump.ignore;
    const dumpFile = path.join(rundir, `${this.type}_config.json`);
    try {
      /* istanbul ignore if */
      if (!fs.existsSync(rundir)) fs.mkdirSync(rundir);
      const json = { config: this.config, plugins: this.plugins };
      const output = JSON.stringify(json, (key, value) => {
        if (ignoreList.indexOf(key) !== -1) return '<ignore>';
