...
const ContextFetch = require('./core/context_fetch');
const Messenger = require('./core/messenger');
const HttpClient = require('./core/httpclient');
const HttpClientNext = require('./core/httpclient_next');
const { createFetchInstances } = require('./core/fetch_factory');
...
const DNSCacheResolver = require('./core/dnscache');

const HTTPCLIENT = Symbol('EggApplication#httpclient');
const FETCH = Symbol('EggApplication#fetch');
const SAFE_FETCH = Symbol('EggApplication#safeFetch');
...
class EggApplication extends EggCore {
  ...
  createHttpClient(options = {}) {
    ...
    // Bind dns lookup function if not already provided
    const enabledDNSCache = this.config.httpclient.enableDNSCache === true;
    if (enabledDNSCache) {
      options.lookup = options.lookup ?? this.dnsResolver.getLookupFunction();
    }
  ...
  get dnsResolver() {
    ...
    if (!this._dnsResolver) {
      const useDNSResolver = this.config.httpclient.useDNSResolver === true;
      this._dnsResolver = new DNSCacheResolver({
        useResolver: useDNSResolver,
        servers: this.config.httpclient.dnsServers,
        max: this.config.httpclient.dnsCacheMaxLength || 1000,
        dnsCacheLookupInterval: this.config.httpclient.dnsCacheLookupInterval,
        addressRotation: this.config.httpclient.dnsAddressRotation,
      });
    }
  ...
  initFetch() {
    ...
    const { fetch, safeFetch } = createFetchInstances(options, this.config);
    this[FETCH] = fetch;
    this[SAFE_FETCH] = safeFetch;
  }

  /**
   * Fetch instance based on urllib4's FetchFactory
   * @see https://github.com/node-modules/urllib
   * @member {Function}
   */
  get fetch() {
    if (!utils.detectFetchVersion()) {
      this.coreLogger.warn('[egg:core] Fetch only works on Node.js >= 20');
      return null;
    }
    if (!this[FETCH]) {
      this.initFetch();
  ...
  /**
   * SafeFetch instance with SSRF protection
   * @see https://github.com/node-modules/urllib
   * @member {Function}
   */
  get safeFetch() {
