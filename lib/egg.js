...
class EggApplication extends EggCore {
  ...
  constructor(options = {}) {
    ...
    this.cluster = (clientClass, options) => {
      options = Object.assign({}, this.config.clusterClient, options, {
        ...
        isCheckHeartbeat:
          this.config.env === "prod" ? true : require("node:inspector").url() === undefined,
      });
  ...
  dumpConfigToObject() {
    ...
    const json = extend(
      true,
      {},
      { config: this.config, plugins: this.loader.allPlugins, appInfo: this.loader.appInfo },
    );
      ...
      for (const item of items) {
        ...
        if (item.index > 0 && item.duration >= this.config.dump.timing.slowBootActionMinDuration) {
          this.coreLogger.warn(
            "[egg:core][slow-boot-action] #%d %dms, name: %s",
            item.index,
            item.duration,
            item.name,
          );
