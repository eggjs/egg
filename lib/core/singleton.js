...
class Singleton {
  ...
  async initAsync() {
    ...
    if (options.clients) {
      await Promise.all(
        Object.keys(options.clients).map((id) => {
          return this.createInstanceAsync(options.clients[id], id).then((client) =>
            this.clients.set(id, client),
          );
        }),
      );
  ...
  _extendDynamicMethods(client) {
    ...
    assert(
      ...
      "singleton instance should not have createInstanceAsync method",
    );
    ...
    try {
      ...
      this.app.logger.warn(
        "egg:singleton %s dynamic create is disabled because of client is unextensible",
        this.name,
      );
    }
  }
