...
class HttpClient extends urllib.HttpClient2 {
  constructor(app, opts = {}) {
    ...
    // Merge defaults with user overrides so lookup and other init options are honored.
    const defaultArgs = {
      ...config.request,
      ...(opts.defaultArgs || opts.request || {}),
    ...
    const options = {
      app,
      defaultArgs,
      agent: opts.agent || new Agent(config.httpAgent),
      httpsAgent: opts.httpsAgent || new HttpsAgent(config.httpsAgent),
      lookup: opts.lookup,
    };
  ...
  get dnsCache() {
    ...
    return this.app.dnsResolver.getDnsCache();
  }
}
