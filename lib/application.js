...
class Application extends EggApplication {
  ...
  _onServerTimeout(socket) {
    ...
    if (req) {
      this.logger.warn('A request `%s %s` timeout with client (%s:%d)', req.method, req.url, socket.remoteAddress, socket.remotePort);
    }
    socket.destroy();
  }
  ...
  onServer(server) {
    ...
    server.setTimeout(this.config.serverTimeout, socket => this._onServerTimeout(socket));
