...
export class EntryGenerator {
  ...
  #addEntry(map: Map<string, BundleEntry>, relKey: string): void {
    ...
    map.set(normalized, { relKey: normalized, absBundle });
  }
  ...
  #renderWorkerEntry(entries: BundleEntry[], manifest: StartupManifest): string {
    ...
    entries.forEach((entry, idx) => {
      ...
      importLines.push(`import * as __m${idx} from ${JSON.stringify(specifier)};`);
      mapLines.push(`  [${JSON.stringify(entry.relKey)}]: __m${idx},`);
    });
...
const __baseDir = typeof __dirname === 'string'
