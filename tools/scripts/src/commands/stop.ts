...
export default class Stop<T extends typeof Stop> extends BaseCommand<T> {
  ...
  public async run(): Promise<void> {
    ...
    this.log(
      `stopping egg application${
        flags.title ? ` with --title=${flags.title}` : ""
      }`,
    );
    ...
    processList = await this.findNodeProcesses((item) => {
      const cmd = item.cmd;
      const matched = flags.title
        ? (osRelated.appWorkerPath.test(cmd) ||
            osRelated.agentWorkerPath.test(cmd)) &&
          cmd.includes(format(osRelated.titleTemplate, flags.title))
        : osRelated.appWorkerPath.test(cmd) ||
          osRelated.agentWorkerPath.test(cmd);
      if (matched) {
  ...
  protected async findNodeProcesses(
    filter: (item: NodeProcess) => boolean,
  ): Promise<NodeProcess[]> {
