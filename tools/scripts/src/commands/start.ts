...
export default class Start<T extends typeof Start> extends BaseCommand<T> {
  ...
  static override flags = {
    title: Flags.string({
      description:
        "process title description, use for kill grep, default to `egg-server-${APP_NAME}`",
    }),
    ...
    sourcemap: Flags.boolean({
      summary:
        "whether enable sourcemap support, will load `source-map-support` etc",
  ...
  protected async getServerBin(): Promise<string> {
    const serverBinName = this.isESM
      ? "start-cluster.mjs"
      : "start-cluster.cjs";
  ...
  public async run(): Promise<void> {
    ...
    this.env.NODE_LOG_DIR =
      this.env.NODE_LOG_DIR || path.join(logDir, "alinode");
    ...
    if (scriptsConfig?.require) {
      scriptsConfig.require = Array.isArray(scriptsConfig.require)
        ? scriptsConfig.require
        : [scriptsConfig.require];
    ...
    if (flags.sourcemap) {
      const sourceMapSupportPkgPath = importResolve(
      ...
      const sourceMapSupport = path.join(
        path.dirname(sourceMapSupportPkgPath),
        "register.js",
      );
      if (this.isESM) {
    ...
    debug(
      "flags: %o, framework: %o, baseDir: %o, execArgv: %o",
      flags,
      frameworkName,
      baseDir,
      execArgv,
