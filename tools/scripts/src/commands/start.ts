...
export default class Start<T extends typeof Start> extends BaseCommand<T> {
  ...
  static override flags = {
    ...
    single: Flags.boolean({
      description: 'start as single process mode (no cluster)',
      default: false,
    }),
  };
  ...
  public async run(): Promise<void> {
    ...
    // In single mode, Node.js 22.18+ native type stripping handles .ts files.
    const isSingle = flags.single;
    if (!isSingle) {
    ...
    const ignoreKeys = ['env', 'daemon', 'stdout', 'stderr', 'timeout', 'ignore-stderr', 'node', 'single'];
    if (isSingle) {
    ...
    if (flags.daemon) {
      ...
      await this.waitForReady(child, frameworkName);
    } else {
