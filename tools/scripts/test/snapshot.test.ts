...
describe.sequential('test/snapshot.test.ts', () => {
  ...
  describe('start with --snapshot-blob', () => {
  ...
  // (needed by @eggjs/tegg). tsx/esbuild don't support it. Use compiled dist/ in production.
  // Will be fixed when tegg migrates to TC39 decorators or swc loader is added.
  ...
  // TODO: Performance comparison requires --single mode cold start, which needs
  // emitDecoratorMetadata support. Skipped until single mode TS loading is resolved.
  describe.skip('snapshot startup performance', () => {
    ...
    it('should start faster from snapshot than cold start', async () => {
      // Cold start (single mode, no snapshot)
      const port1 = await detectPort();
      await fs.rm(path.join(logDir, 'master-stderr.log'), { force: true });
      ...
      // Verify snapshot start works
      const snapResult = await request(`http://127.0.0.1:${port2}`);
      assert.equal(snapResult.status, 200);

      // Log timing comparison (informational, not a hard assertion in CI)
      ...
      // Soft assertion: snapshot should generally be faster.
      // We don't hard-fail because CI timing can be noisy.
