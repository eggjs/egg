...
describe("test/start.test.ts", () => {
  ...
  describe("start with daemon", () => {
    ...
    beforeEach(async () => {
      ...
      await fs.writeFile(
        path.join(logDir, "master-stderr.log"),
        "just for test",
      );
    });
    ...
    it("should start custom-framework", async () => {
      ...
      assert(
        fileList.some((name) => name.match(/master-stdout\.log\.\d+\.\d+/)),
      );
      assert(
        fileList.some((name) => name.match(/master-stderr\.log\.\d+\.\d+/)),
      );
  ...
  describe("check status", () => {
    ...
    it("should status check fail `--ignore-stderr`, exit with 0", async () => {
      ...
      const app = coffee.fork(
        eggBin,
        ["start", "--daemon", "--workers=1", "--ignore-stderr"],
        { cwd },
      );
      // app.debug();
