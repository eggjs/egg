...
The repository is ESM-first and TypeScript-heavy; prefer `.ts` sources and exports over CommonJS. Code formatting is enforced by `oxfmt` (for JS/TS files) and `oxlint --type-aware` (for linting)—two-space indentation, trailing commas, and semicolons are the defaults. Name files in lowercase with hyphens (e.g. `loader-context.ts`), classes in PascalCase, and functions/variables in camelCase. Re-export types thoughtfully to keep the public API stable.
