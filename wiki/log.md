...
- sources touched: `.github/workflows/pkg-pr-new.yml`, `scripts/publish-preview.js`, `scripts/utils.js`, `CONTRIBUTING.md`, `CONTRIBUTING.zh-CN.md`, `site/docs/community/{index,pr-preview-packages}.md`, `site/docs/zh-CN/community/{index,pr-preview-packages}.md`, `site/.vitepress/config.mts`, `https://github.com/stackblitz-labs/pkg.pr.new`
- pages updated: `wiki/workflows/pr-preview-packages.md`, `wiki/index.md`, `wiki/log.md`
- note: The `pkg.pr.new` label enables preview publication for public workspace packages. Later commits and reopened PRs publish while the label remains attached. The workflow accepts stacked PRs, builds with utoo, and runs `pkg-pr-new@latest`. The preview script reuses the npm release helpers to resolve catalogs and published exports, then restores each manifest. Directory inputs preserve internal preview dependency links. Publication uses a read-only workflow token and the pkg.pr.new GitHub App. English and Chinese site guides cover installation, PR and commit URLs, lockfiles, and restoring regular dependencies, with links from the version menu, Community sidebar and landing page, and contribution guides.
