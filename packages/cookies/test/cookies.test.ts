...
describe("test/cookies.test.ts", () => {
  ...
  it("should work with domain is empty string", () => {
    ...
    assert.equal(
      cookie,
      "foo=SBfi5dnMSwNmMdeydI_zdw==; path=/; secure; httponly",
    );
  });
  ...
  it("should update .sig if not match the first key", () => {
    ...
    assert(
      newCookies.ctx.response.headers["set-cookie"][0].startsWith(
        `foo.sig=${newSign}`,
      ),
    );
  });
  ...
  it("should not overwrite default", () => {
    ...
    assert(
      cookies.ctx.response.headers["set-cookie"].join(";").match(/foo=bar/),
    );
  });
  ...
  it("should overwrite when opts.overwrite = true", () => {
    ...
    assert(
      cookies.ctx.response.headers["set-cookie"].join(";").match(/foo=hello/),
    );
  });
  ...
  it("should add secure when ctx.secure = true", () => {
    ...
    cookies.set("foo", "bar");
    assert(
      cookies.ctx.response.headers["set-cookie"].join(";").match(/secure;/),
    );
  });
  ...
  it("should throw when ctx.secure = false but opt.secure = true", () => {
    ...
    try {
      ...
      assert(
        err.message === "Cannot send secure cookie over unencrypted connection",
      );
    }
  });
  ...
  it("should set cookie success when set-cookie already exist", () => {
    ...
    assert(
      cookies.ctx.response.headers["set-cookie"][1] ===
  ...
  it("should emit cookieLimitExceed event in app when value length exceed the limit", () => {
    ...
    expect(cookies.ctx.response.headers["set-cookie"][0]).toMatch(
      /^foo=1{4094}; path=\/; httponly$/,
    );
  });
  ...
  describe("opts.partitioned", () => {
    ...
    it("should send partitioned property on Chrome >= 118", () => {
      ...
      for (const str of cookies.ctx.response.headers["set-cookie"]) {
        assert(
          str.includes(
            "; path=/; samesite=none; secure; httponly; partitioned",
          ),
        );
      }
    ...
    it("should not set partitioned property when secure = false", () => {
      ...
      assert.equal(
        headers[1],
        "foo.sig=ZWbaA4bWk8ByBuYVgfmJ2DMvhhS3sOctMbfXAQ2vnwI; path=/; httponly",
      );
    });
    ...
    it("should not set partitioned property when sameSite != none", () => {
      ...
      assert.equal(
        headers[1],
        "foo.sig=ZWbaA4bWk8ByBuYVgfmJ2DMvhhS3sOctMbfXAQ2vnwI; path=/; secure; httponly",
      );
    });
  });
      ...
      cookies = Cookies(
        {
          ...
          headers: {
            cookie:
              "_CHIPS-foo=hello; _CHIPS-foo.sig=G4Idm9Wdp_vfCnUbOpQG284o22SgTe88SUmG6QW1ylk",
          },
        },
        { secure: true },
        { autoChips: true },
      ...
      cookies = Cookies(
        {
          ...
          headers: {
            cookie:
              "_CHIPS-foo=hello; _CHIPS-foo.sig=G4Idm9Wdp_vfCnUbOpQG284o22SgTe88SUmG6QW1ylk-invalid",
          },
        },
        { secure: true },
        { autoChips: true },
