...
describe('app.respond', () => {
  ...
  describe('when .body is a Response', () => {
    it('should keep Response headers', async () => {
      const app = new Koa();
    ...
    it('should respond with body content', async () => {
      ...
      const res = await request(app.callback()).get('/').expect(200).expect('content-type', 'text/plain');
    ...
    it('should handle Response from fetch() with JSON', async () => {
      ...
      const res = await request(app.callback()).get('/').expect(200).expect('content-type', 'application/json');
  ...
  describe('when .body is a ReadableStream', () => {
    it('should respond', async () => {
      ...
      return request(app.callback()).head('/').expect(200).expect('content-type', 'application/octet-stream');
    });
