...
describe("test/EggRouter.test.ts", () => {
  ...
  it("should throw error on generator function", () => {
    ...
    expect(() => {
      ...
      /post `\/hello\/world`: Please use async function instead of generator function/,
    );
  });
  ...
  it("should router.url work", () => {
    ...
    expect(router.url("post", { id: 1, foo: [1, 2], bar: "bar" })).toBe(
      "/post/1?foo=1&foo=2&bar=bar",
    );
    expect(router.url("post", { foo: [1, 2], bar: "bar" })).toBe(
      "/post/:id?foo=1&foo=2&bar=bar",
    );
    expect(router.url("fooo")).toBe("");
    expect(router.url("hello")).toBe("/hello/world");

    expect(router.pathFor("post", { id: 1, foo: [1, 2], bar: "bar" })).toBe(
      "/post/1?foo=1&foo=2&bar=bar",
    );
