...
const generateOpts = { serverEnv: 'prod', serverScope: '', typescriptEnabled: true } as const;
...
describe('ManifestStore fingerprint stability', () => {
  ...
  it('should produce same fingerprint for unchanged file', () => {
    ...
    const m1 = ManifestStore.createCollector(tmpDir).generateManifest(generateOpts);
    const m2 = ManifestStore.createCollector(tmpDir).generateManifest(generateOpts);
  ...
  it('should change config fingerprint when file is added', async () => {
    ...
    try {
      const m1 = ManifestStore.createCollector(baseDir).generateManifest(generateOpts);
      await new Promise((resolve) => setTimeout(resolve, 50));
      fs.writeFileSync(path.join(baseDir, 'config', 'b.ts'), 'const b = 2;');
      const m2 = ManifestStore.createCollector(baseDir).generateManifest(generateOpts);
  ...
  it('should handle symlink cycles without infinite recursion', () => {
    ...
    try {
      ...
      const manifest = ManifestStore.createCollector(baseDir).generateManifest(generateOpts);
