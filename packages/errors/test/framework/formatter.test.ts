...
describe("test/framework/formatter.test.ts", () => {
  ...
  describe("format", () => {
    it("should format FrameworkError", () => {
      ...
      expect(message).toContain(
        "framework.CustomError: error [ https://eggjs.org/zh-cn/faq/customPlugin_00 ]",
      );
    ...
    it("should use faqPrefixEnv", () => {
      ...
      vi.spyOn(FrameworkErrorFormatter, "faqPrefixEnv", "get").mockReturnValue(
        "https://www.custom.com/faq",
      );
      const err = new CustomError("error", "00");
      const message = FrameworkErrorFormatter.format(err);
      expect(message).toContain(
        "framework.CustomError: error [ https://www.custom.com/faq/customPlugin_00 ]",
      );
    });
  });
  ...
  describe("extendable", () => {
    it("custom formatter should work", () => {
      ...
      expect(message).toContain(
        "framework.CustomError: error [ http://custom/faq/customPlugin_00 ]",
      );
    });
  });
  ...
  describe("formatError", () => {
    it("should format FrameworkError", () => {
      ...
      expect(err.message).toBe(
        "error [ https://eggjs.org/zh-cn/faq/customPlugin_00 ]",
      );
    });
    ...
    it("should use faqPrefixEnv", () => {
      ...
      err = FrameworkErrorFormatter.formatError(err);
      expect(err.message).toBe(
        "error [ https://www.custom.com/faq/customPlugin_00 ]",
      );
    });
