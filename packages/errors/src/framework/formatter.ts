...
export class FrameworkErrorFormatter {
  ...
  private static faqPrefixEnv =
    process.env.EGG_FRAMEWORK_ERR_FAQ_PREFIX ??
    process.env.EGG_FRAMEWORK_ERR_FAQ_PERFIX;

  static format(err: Error): string {
    const faqPrefix = this.faqPrefixEnv ?? this.faqPrefix;
    let errMessage = err.message;
    if (
      FrameworkBaseError.isFrameworkError(err) &&
      !errMessage.includes(faqPrefix)
    ) {
  ...
  static formatError<T extends Error>(err: T): T {
    ...
    if (
      FrameworkBaseError.isFrameworkError(err) &&
      !err.message.includes(faqPrefix)
    ) {
