  ...
  describe("ctx.__(key, value)", () => {
    it("should return locale de", async () => {
      await app
        ...
        .expect(
          "Set-Cookie",
          /,locale=de; path=\/; max-age=31557600; expires=[^;]+ GMT$/,
        )
        .expect({
    ...
    it("should __() work", () => {
      ...
      expect(ctx.__("Email %s %d %j", "ok", 1, { foo: "bar" })).toBe(
        'Email ok 1 {"foo":"bar"}',
      );
      expect(ctx.__("Email %s %d %j %s", "ok", 1, { foo: "bar" }, "foo")).toBe(
        'Email ok 1 {"foo":"bar"} foo',
      );
      expect(ctx.__("Email {a} {b}", { a: "a", b: "b" })).toBe("Email a b");
      expect(ctx.__("Email {a} {b} {c}", { a: "a", b: "b" })).toBe(
        "Email a b {c}",
      );
      expect(ctx.__("Email {0} {1}", ["a", "b"])).toBe("Email a b");
      expect(ctx.app.__("en-us", "Email {0} {1}", ["a", "b"])).toBe(
        "Email a b",
      );
  ...
  describe("with cookieDomain", () => {
    ...
    it("should return locale de", async () => {
      await app
        ...
        .expect(
          "Set-Cookie",
          /locale=de; path=\/; max-age=31557600; expires=[^;]+ GMT; domain=.foo.com$/,
        )
        .expect({
          message: "Hallo fengmk2, wie geht es dir heute? Wie war dein 18.",
          empty: "",
    ...
    it("should return default locale en_US", async () => {
      await app
        ...
        .expect(
          "Set-Cookie",
          /locale=en-us; path=\/; max-age=31557600; expires=[^;]+ GMT; domain=.foo.com$/,
        )
        .expect({
          message: "Hello fengmk2, how are you today? How was your 18.",
          empty: "",
