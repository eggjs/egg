...
describe("test/onerror.test.ts", () => {
  ...
  it("should return jsonp style", async () => {
    ...
    await app
      ...
      .expect(
        '/**/ typeof fn === \'function\' && fn({"message":"Internal Server Error"});',
      )
      .expect(500);
  });
  ...
  if (process.platform === "linux") {
    ...
    it("should log warn 4xx", async () => {
      ...
      const warnLog = getFixtures(
        "onerror-4xx/logs/onerror-4xx/onerror-4xx-web.log",
      );
      const content = fs.readFileSync(warnLog, "utf8");
      assert.match(
        content,
        /POST \/body_parser] nodejs\..*?Error: request entity too large/,
      );
    });
  }
