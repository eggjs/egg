...
describe("test/file-mode.test.ts", () => {
  ...
  it("should upload", async () => {
    ...
    expect(data.files[0].filepath.startsWith(app.config.multipart.tmpdir)).toBe(
      true,
    );
    ...
    expect(data.files[1].filepath.startsWith(app.config.multipart.tmpdir)).toBe(
      true,
    );
    ...
    expect(data.files[2].filepath.startsWith(app.config.multipart.tmpdir)).toBe(
      true,
    );
  });
  ...
  it("should throw error when file name invalid", async () => {
    ...
    expect(res.data.toString()).toMatch(
      /Error: Invalid filename: foooooooo.js.rar/,
    );
  });
  ...
  describe("schedule/clean_tmpdir", () => {
    it("should register clean_tmpdir schedule", async () => {
      ...
      expect(content).toMatch(
        /\[@eggjs\/schedule\]: register schedule .+clean_tmpdir\.ts/,
      );
    });

    it("should remove nothing", async () => {
      app.mockLog();
      await app.runSchedule(
        path.join(import.meta.dirname, "../src/app/schedule/clean_tmpdir"),
      );
      await scheduler.wait(1000);
      app.expectLog(
        '[@eggjs/multipart:CleanTmpdir] start clean tmpdir: "',
        "coreLogger",
      );
    ...
    it("should remove old dirs", async () => {
      const oldDirs = [
        path.join(
          ...
          dayjs().subtract(1, "years").format("YYYY/MM/DD/HH"),
        ),
        path.join(
          app.config.multipart.tmpdir,
          dayjs().subtract(1, "months").format("YYYY/MM/DD/HH"),
        ),
        path.join(
          app.config.multipart.tmpdir,
          dayjs().subtract(2, "months").format("YYYY/MM/DD/HH"),
        ),
        path.join(
          app.config.multipart.tmpdir,
          dayjs().subtract(3, "months").format("YYYY/MM/DD/HH"),
        ),
        path.join(
          app.config.multipart.tmpdir,
          dayjs().subtract(1, "days").format("YYYY/MM/DD/HH"),
        ),
        path.join(
          app.config.multipart.tmpdir,
          dayjs().subtract(7, "days").format("YYYY/MM/DD/HH"),
        ),
      ];
      const shouldKeepDirs = [
        path.join(
          app.config.multipart.tmpdir,
          dayjs().subtract(2, "years").format("YYYY/MM/DD/HH"),
        ),
      ...
      await Promise.all(
        oldDirs.map((dir) => fs.mkdir(dir, { recursive: true })),
      );
      await Promise.all(
        shouldKeepDirs.map((dir) => fs.mkdir(dir, { recursive: true })),
      );
      ...
      app.expectLog(
        '[@eggjs/multipart:CleanTmpdir] removing tmpdir: "',
        "coreLogger",
      );
      app.expectLog(
        '[@eggjs/multipart:CleanTmpdir:success] tmpdir: "',
        "coreLogger",
      );
    });
  });
