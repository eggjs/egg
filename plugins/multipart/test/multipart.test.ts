    ...
    it("should not has clean_tmpdir schedule", async () => {
      try {
        ...
        expect(err.message).toBe(
          "[@eggjs/schedule] Cannot find schedule clean_tmpdir",
        );
      }
    });
  ...
  describe("upload over fileSize limit", () => {
    ...
    it("should show error", async () => {
      ...
      expect(content).toContain(
        "nodejs.MultipartFileTooLargeError: Request file too large",
      );
    ...
    it("should ignore stream next errors after limit event fire", async () => {
      ...
      form.file(
        "file",
        bigfile,
        "not-handle-error-event-and-mock-stream-error.js",
      );
