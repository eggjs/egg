  ...
  if (path.includes("%")) {
    try {
      ...
      if (ctx.app.config.env === "local" || ctx.app.config.env === "unittest") {
        ...
        ctx.coreLogger.warn(
          "[@eggjs/security: dta global block] : decode file path %j failed.",
          path,
        );
      }
    }
...
export function preprocessConfig(config: SecurityConfig): void {
  ...
  config.domainWhiteList = config.domainWhiteList.map((domain: string) =>
    domain.toLowerCase(),
  );
  ...
  if (config.csrf && config.csrf.refererWhiteList) {
    config.csrf.refererWhiteList = config.csrf.refererWhiteList.map(
