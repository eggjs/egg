...
export type HttpClientResponse<T = any> = Awaited<
  ReturnType<HttpClient["prototype"]["request"]>
> & { data: T };
...
export async function safeCurlForApplication<T = any>(
  ...
  if (ssrfConfig?.checkAddress) {
    ...
    app.logger.warn(
      "[@eggjs/security] please configure `config.security.ssrf` first",
    );
  }

  if (ssrfConfig?.checkAddress) {
    let httpClient = app[SSRF_HTTPCLIENT] as ReturnType<
      EggApplicationCore["createHttpClient"]
    >;
