...
export default (_: unknown, app: Application): MiddlewareFunc => {
  ...
  if (options.match || options.ignore) {
    app.coreLogger.warn(
      "[@eggjs/security/middleware/securities] Please set `match` or `ignore` on sub config",
    );
  }
  ...
  defaultMiddlewares.forEach((middlewareName: SecurityMiddlewareName) => {
    ...
    if (middlewareName === "csrf" && opt.useSession && !app.plugins.session) {
      throw new Error(
        "csrf.useSession enabled, but session plugin is disabled",
      );
    }
    ...
    const createMiddleware =
      securityMiddlewares[middlewareName as keyof typeof securityMiddlewares];
    const fn = createMiddleware(opt);
    middlewares.push(fn);
    app.coreLogger.info(
      "[@eggjs/security/middleware/securities] use %s middleware",
      middlewareName,
    );
  });
