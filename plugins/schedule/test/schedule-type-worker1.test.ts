...
describe("cluster - worker-ctxStorage", () => {
  ...
  it("should work", async () => {
    ...
    // unstable
    expect(
      contains(scheduleLog, "foobar.js executing by app"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      contains(scheduleLog, "foobar.js execute succeed"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      contains(scheduleLog, "interval.js executing by app"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      contains(scheduleLog, "interval.js execute succeed"),
    ).toBeGreaterThanOrEqual(0);
  });
});
