...
The snapshot feature was implemented **without utoo bundling**. Instead, the approach uses:

1. **`generate-snapshot-entry.mjs`**: Scans the egg app at build time to discover all dynamically-loaded modules, then generates a static entry file with explicit imports for every module
2. **esbuild**: Bundles the generated entry into a single CJS file (required because `--build-snapshot` only supports CJS), with custom plugins for `urllib` (stub), `node:http` (lazy proxy), and `file://` URL resolution
3. **V8 snapshot**: Captures the entire initialized application state in the heap

This approach avoids the utoo bundler entirely while achieving the same goal: all modules are pre-loaded in the snapshot. The esbuild step is purely a format conversion (ESM → CJS), not an application bundling step. See `docs/snapshot-design.md` for the full architecture.
