...
describe("test/app/extend/helper.test.js", () => {
  ...
  describe("pathFor()", () => {
    ...
    it("should get home path with params", () => {
      return app.httpRequest().get("/pathFor?foo=bar").expect("/home?foo=bar").expect(200);
    });
  });
  ...
  describe("escape()", () => {
    it("should escape script", () => {
      return app.httpRequest().get("/escape").expect("&lt;script&gt;").expect(200);
    });
  });

  describe("shtml()", () => {
    it("should ignore attribute if domain not in domainWhiteList", () => {
      return app.httpRequest().get("/shtml-not-in-domain-whitelist").expect("true").expect(200);
    });
  });
});
