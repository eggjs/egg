...
describe("test/app/extend/application.test.js", () => {
  ...
  describe("app.addSingleton()", () => {
    ...
    it("should add singleton success", async () => {
      ...
      try {
        ...
        assert(
          err.message ===
            "egg:singleton dataServiceAsync only support create asynchronous, please use createInstanceAsync",
        );
      }
  ...
  describe("app.runInAnonymousContextScope(scope)", () => {
    it("should run task in anonymous context scope success", async () => {
      ...
      const logdir = app.config.logger.dir;
      const logs = fs
        .readFileSync(path.join(logdir, "app-runInAnonymousContextScope-web.log"), "utf8")
        .split("\n");
      // console.log(logs);
  ...
  describe("app.runInAnonymousContextScope(scope,request)", () => {
    it("should run task in anonymous context scope with req success", async () => {
      ...
      const logdir = app.config.logger.dir;
      const logs = fs
        .readFileSync(path.join(logdir, "app-runInAnonymousContextScope-withRequest-web.log"), {
          encoding: "utf8",
        })
        .split("\n");
