...
describe("test/ts/index.test.js", () => {
  describe("compiler code", () => {
    ...
    before(async () => {
      await coffee
        .fork(require.resolve("typescript/bin/tsc"), [
          "-p",
          path.resolve(__dirname, "../fixtures/apps/app-ts/tsconfig.json"),
        ])
        .debug()
        .expect("code", 0)
        .end();
    ...
    it("controller run ok", (done) => {
      app.httpRequest().get("/foo").expect(200).expect({ env: "unittest" }).end(done);
    });

    it("controller of app.router run ok", (done) => {
      app.httpRequest().get("/test").expect(200).expect({ env: "unittest" }).end(done);
    });
  });
  ...
  describe("type check", () => {
    it("should compile with esModuleInterop without error", async () => {
      await coffee
        .fork(require.resolve("typescript/bin/tsc"), [
          "-p",
          path.resolve(__dirname, "../fixtures/apps/app-ts-esm/tsconfig.json"),
        ])
        .debug()
        .expect("code", 0)
        .end();
    ...
    it("should compile type-check ts without error", async () => {
      await coffee
        .fork(require.resolve("typescript/bin/tsc"), [
          "-p",
          path.resolve(__dirname, "../fixtures/apps/app-ts-type-check/tsconfig.json"),
        ])
        .debug()
        .expect("code", 0)
        .end();
    ...
    it("should throw error with type-check-error ts", async () => {
      await coffee
        .fork(require.resolve("typescript/bin/tsc"), [
          "-p",
          path.resolve(__dirname, "../fixtures/apps/app-ts-type-check/tsconfig-error.json"),
        ])
        // .debug()
        ...
        .expect(
          "stdout",
          /Property 'directory' is missing in type '{}' but required in type 'CustomLoaderConfig'/,
        )
