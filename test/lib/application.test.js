...
describe("test/lib/application.test.js", () => {
  ...
  describe("handle uncaughtException", () => {
    ...
    it("should handle uncaughtException and log it", async () => {
      ...
      const logfile = path.join(
        utils.getFilepath("apps/app-throw"),
  ...
  describe("warn confused configurations", () => {
    it("should warn if confused configurations exist", async () => {
      ...
      assert(
        logs.match(/Unexpected config key `bodyparser` exists, Please use `bodyParser` instead\./),
      );
      assert(
        logs.match(/Unexpected config key `notFound` exists, Please use `notfound` instead\./),
      );
      assert(
        logs.match(/Unexpected config key `sitefile` exists, Please use `siteFile` instead\./),
      );
      assert(
        logs.match(/Unexpected config key `middlewares` exists, Please use `middleware` instead\./),
      );
      assert(
        logs.match(/Unexpected config key `httpClient` exists, Please use `httpclient` instead\./),
      );
    });
  });
  ...
  describe("test on apps/demo", () => {
    ...
    describe("request and response event", () => {
      it("should emit when request success", (done) => {
        ...
        app.httpRequest().get("/class-controller").expect("this is bar!").expect(200, done);
      });
