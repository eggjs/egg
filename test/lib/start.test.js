...
describe("test/lib/start.test.js", () => {
  ...
  describe("start", () => {
    ...
    it("should request work", async () => {
      app = await utils.singleProcessApp("apps/demo");
      await app.httpRequest().get("/protocol").expect(200).expect("http");

      await app.httpRequest().get("/class-controller").expect(200).expect("this is bar!");
    });
  ...
  describe("custom framework work", () => {
    it("should work with options.framework", async () => {
      app = await utils.singleProcessApp("apps/demo", {
        framework: path.join(__dirname, "../fixtures/custom-egg"),
      });
